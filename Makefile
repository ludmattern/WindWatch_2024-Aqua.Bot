...
BASH_ROS_SOURCE = $(ROS_SETUP).bash
BASH_VRX_SOURCE = $(VRX_SETUP).bash
...
run:
	...
	ros2 launch aquabot_gz competition.launch.py world:=$(DEFAULT_WORLD) competition_mode:=false > /dev/null &

.PHONY: launch_mission
launch_mission:
	@echo "\e[32m[INFO] Launching the complete mission (all packages)...\e[0m"
	ros2 launch mission_manager mission_launch.py

.PHONY: launch_headless
launch_headless:
	@echo "\e[32m[INFO] Launching the mission in headless mode...\e[0m"
	ros2 launch mission_manager mission_launch.py headless:=true
...
perception:
	...
	ros2 run sensors camera_processing_node
...
rviz:
	...
	ros2 launch visualization rviz.launch.py > /dev/null &
