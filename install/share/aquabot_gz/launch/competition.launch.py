...
def launch(context, *args, **kwargs):
    ...
    if config_file and config_file != '':
      ...
      m = Model('aquabot', 'wam-v', [0, 0, 0, 0, 0, 1])
      if robot_urdf and robot_urdf != '':
          m.set_urdf(robot_urdf)
      else:
          robot_urdf = os.path.join(get_package_share_directory('aquabot_description'), 'urdf/aquabot.urdf')
          m.set_urdf(robot_urdf)
      models.append(m)

    # Needs to copy model.urdf to avoid error on first launch (wamv model name is hard defined in spawn function)
    wamv_urdf_file = os.path.join(get_package_share_directory('vrx_gazebo'), 'models/wamv/tmp/model.urdf')
    if not os.path.exists(wamv_urdf_file):
        os.makedirs(os.path.dirname(wamv_urdf_file), exist_ok=True)
        os.system('cp ' + robot_urdf + ' ' + wamv_urdf_file)
    ...
    if (sim_mode == 'bridge' or sim_mode == 'full') and bridge_competition_topics:
        launch_processes.extend(aquabot_gz.launch.competition_bridges(world_name_base, competition_mode))
