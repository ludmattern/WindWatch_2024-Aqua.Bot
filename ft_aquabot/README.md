...
- **Nœud `tgt_pos_update_node`**
  ...
  - **Topics Souscrits** :
    - `/aquabot/ais_sensor/windturbines_positions` : Liste des positions GPS des éoliennes.
  - **Topics Publiés** :
    - `/aqua_bot/ais_sensor/target_positions` : Positions des éoliennes.

#### 3. Package `mission_control`

Ce package gère la **planification des missions** et les **objectifs à atteindre**.
...
#### 4. Package `mission_coordinator`

Ce package est chargé de coordonner la **mission** et de superviser les différentes phases.
...
4. **Topic `/mission/target_status`**
   ...
   - **Publié Par** : `camera_processing_node`.
   - **Souscrit Par** : `mission_coordinator_node`.

5. **Topic `/mission/avoidance_course`**
   - **Description** : Modifications de trajectoire pour éviter les obstacles.
   - **Publié Par** : `obstacle_avoidance_node`.
   - **Souscrit Par** : `navigation_node`.

6. **Topic `/mission/mission_goal`**
   - **Description** : Objectifs tactiques à atteindre.
   - **Publié Par** : `mission_coordinator_node`.
   - **Souscrit Par** : `navigation_node`.

7. **Topic `/aqua_bot/ais_sensor/vessel_positions`**
   - **Description** : Contient les positions des navires environnants, afin d'éviter toute collision.
   - **Publié Par** : AIS (Automated Identification System).
   - **Souscrit Par** : `navigation_node`, `obstacle_avoidance_node`, `visualization_node`.

8. **Topic `/aqua_bot/ais_sensor/target_positions`**
   - **Description** : Positions des éoliennes détectées.
   - **Publié Par** : `tgt_pos_update_node`.
   - **Souscrit Par** : `target_manager_node`, `visualization_node
